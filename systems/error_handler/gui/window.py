...
class ErrorReportWindow(QDialog):
    ...
    """
    def __init__(self, exc_type, exc_value, traceback_str, severity="Critical"):
        super().__init__()
        self.setWindowTitle(f"Application Error [{severity}]")
        ...
        # Header
        color = "#f14c4c" if severity == "Critical" else ("#f5f543" if severity == "Specific" else "#29b8db")
        header_label = QLabel(f"<h3><span style='color:{color};'>[{severity} Error]</span> An unhandled exception occurred:</h3><p>{exc_value}</p>")
        ...
        if self.severity in ["Specific", "Minor"]:
            ignore_btn = QPushButton("Ignore Error")
            ignore_btn.clicked.connect(self.ignore_error)
...
def show_error_dialog(exc_type, exc_value, traceback_str, severity="Critical"):
    """
    Helper function to safely show the dialog even if the app is unstable.
    Returns True if the user chose to ignore the error.
    """
    # Ensure execution on main thread if called from background
    import threading
    ...
    window = ErrorReportWindow(exc_type, exc_value, traceback_str, severity)
    window.exec()
